...
export default function CreatePage() {
  ...
  async function publish(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setError("");
      setStage("signing");
      const session = await clientApi<{ assetId: string; uploadUrl: string; requiredHeaders: Record<string, string> }>("/media/image-uploads", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ contentType: file.type, byteSize: file.size }),
      });
      setStage("uploading");
      const stored = await fetch(session.uploadUrl, { method: "PUT", headers: session.requiredHeaders, body: file });
      if (!stored.ok) throw new Error("스토리지 업로드에 실패했습니다.");
      setStage("verifying");
      await clientApi(`/media/assets/${session.assetId}/complete`, { method: "POST" });
      setStage("publishing");
      const post = await clientApi<{ id: string }>("/posts/images", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ assetId: session.assetId, caption }),
      });
  ...
  const stageLabel = { idle: "게시하기", signing: "업로드 준비 중", uploading: "스토리지에 업로드 중", verifying: "이미지 안전성 확인 중", publishing: "게시물 발행 중" }[stage];

  return (
    <div className="mx-auto max-w-5xl px-4 py-10 pb-28 sm:px-8">
      <div className="mb-8">
        <p className="text-xs font-bold tracking-[0.18em] text-accent">CREATE</p>
        <h1 className="mt-2 text-3xl font-semibold tracking-[-0.04em]">새 이미지 게시물</h1>
        <p className="mt-2 text-sm text-muted">원본은 브라우저에서 스토리지로 바로 전송되고 서버가 실제 파일을 다시 검증합니다.</p>
      </div>
      <form onSubmit={publish} className="grid gap-7 lg:grid-cols-[1.15fr_0.85fr]">
        <label className="relative grid min-h-[28rem] cursor-pointer place-items-center overflow-hidden rounded-[1.75rem] border border-dashed border-line bg-panel transition hover:border-accent">
          {preview ? <Image src={preview} alt="업로드 미리보기" fill unoptimized className="object-contain p-3" /> : <span className="grid place-items-center text-center"><span className="grid size-16 place-items-center rounded-2xl bg-accent-soft text-accent"><ImagePlus className="size-7" /></span><strong className="mt-5 text-lg">이미지를 선택하세요</strong><span className="mt-2 text-sm text-muted">JPEG, PNG, WebP · 최대 20MB</span></span>}
          <input type="file" accept="image/jpeg,image/png,image/webp" onChange={choose} className="sr-only" />
        </label>
