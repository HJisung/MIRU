...
export class CommentsService {
  ...
  list(targetId: string) {
    return this.database.client.engagementComment.findMany({
      where: { targetId },
  ...
  async create(userId: string, targetId: string, body: string) {
    return this.database.client.$transaction(async (transaction) => {
      await this.targets.lockActiveTarget(transaction, targetId);
      const comment = await transaction.engagementComment.create({
        data: { authorId: userId, targetId, body: body.trim() },
        select: {
          id: true,
          body: true,
          createdAt: true,
  ...
  async createLegacy(userId: string, postId: string, body: string) {
